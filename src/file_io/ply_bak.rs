...
use crate::geometry::mesh::{MeshData, Triangle};
...
pub fn load_path(path: &str) -> SimpleResult<Mesh> {
    ...
    let header = try_with!(
        ...
        "Problem when parsing header of ply file at: \"{}\".",
        path
    );
    ...
    if !has_pos || !has_indices {
        bail!("Ply file at \"{}\" is missing positions or indices.", path);
    }

    if has_tan && !has_nrm {
        bail!(
            "Ply file at \"{}\" is missing normals when given tans",
            path
        );
    }
    ...
    let vertices: Vec<f32> = if !has_nrm && !has_tan && !has_uv {
