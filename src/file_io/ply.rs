use crate::geometry::mesh::{TriMesh, Triangle};
use crate::math::vector::{Vec2, Vec3};
use crate::memory::transmute_vec;
...
extern "C" fn error_cb(ply: rply::p_ply, message: *const raw::c_char) {}
...
extern "C" fn index_cb(argument: rply::p_ply_argument) -> raw::c_int {
    ...
    if num_indices != buffer.face_count {
        buffer.proc_status = IndexProcStatus::IncFaceSize;
        return 0;
    }

    if face_index < 0 {
        buffer.proc_status = IndexProcStatus::NegIndex;
        return 1;
    }
...
pub fn load_tri_mesh(path: &str) -> SimpleResult<TriMesh> {
    ...
    let mut indices = IndexBuffer {
        proc_status: IndexProcStatus::NoIssue,
        face_count: 3,
        buffer: Vec::new(),
    };
    ...
    match indices.proc_status {
        IndexProcStatus::NoIssue => (),
        IndexProcStatus::NegIndex => (), // TODO: log it as a warning
        IndexProcStatus::IncFaceSize => {
